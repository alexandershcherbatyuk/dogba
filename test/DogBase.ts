...
describe("DogBase contract", function () {
  async function deployDTBContract() {
   ...
   return {DBT, owner}
    ...
    describe("Deploy", function () {
        it("Initial owner of contract specified", async function () {
            
            const { DBT, owner } = await loadFixture(deployDTBContract);
            
            expect(await DBT.owner()).to.equal(owner.address);
        });
    });

    describe("Dog regestration", function () {
        it("Reg a new dog", async function () {
            const { DBT, owner } = await loadFixture(deployDTBContract);
                    /*const { hardhatToken, owner } = await loadFixture(deployDTBContract);

        const chip = 643194800190471;
        const dog = {
            name : "Maks",
            species : "dog",
            ...
            region : "NSW",
            city : "Sydney",
            addr : "Bennelong Point",
            tatoo : "N/A",
            color : "White, Grey, Black, Orange",
            photo : "",
            birthday : '1675036800',
            postalCode : '50000'
        }
        ...
        const contact = {
            email : "alex@dogba.com",
            cellPhone : '+61292507111',
            otherPhone : '+61292507112',
        }
        ...
        const lostDog = {
            ...
            region : 'NSW',
            city : 'Sydnay',
            addr : "National Park main entrance",
        }
        ...
        console.log("15 digit chip");
        console.log(Math.floor(100000000000000 + Math.random() * 900000000000000));

        await hardhatToken.regDoge(chip, dog, contact);

        await hardhatToken.claimDogIsLost(chip,lostDog);

        await hardhatToken.claimDogIsFound(chip, foundDog, contact);

        console.log(await hardhatToken.dogs(chip));

        console.log(await hardhatToken.lostDogs(chip,0));

        console.log(await hardhatToken.foundDogs(chip,0));

        expect((await hardhatToken.dogs(chip)).name).to.equal(dog.name);
        */
   ...
   /* describe("LostDogs", function () {
        it("Claim dog is missing", async function () {
       ...
       // console.log(await hardhatToken.ownerOf(chip));

        //expect((await hardhatToken.lostDogs(chip,0)).missingDate).to.greaterThan(0);
        });
